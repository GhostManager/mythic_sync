        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{GHOSTWRITER_URL}/oplog/api/entries/", data=json.dumps(gw_message), headers=headers, ssl=False) as resp:
                    if resp.status != 201:
                        mythic_sync_log.error(f"[!] Error posting to Ghostwriter with HTTP code {resp.status}, trying again in {WAIT_TIMEOUT} seconds...")
                        await asyncio.sleep(WAIT_TIMEOUT)
                        continue
        except Exception as e:
            mythic_sync_log.error(f"[!] Exception trying to post the initial entry to Ghostwriter!\n{str(e)}\nTrying again in {WAIT_TIMEOUT} seconds...")
            await asyncio.sleep(WAIT_TIMEOUT)
            continue
        return
...
async def mythic_task_to_ghostwriter_message(message) -> dict:
    ...
    try:
        ...
        mythic_sync_log.error("[!] Failed to process message from Mythic into message for GhostWriter: {}".format(str(e)))
    return gw_message
...
async def createEntry(mythic_instance: mythic_classes.Mythic, message: dict):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{GHOSTWRITER_URL}/oplog/api/entries/", data=json.dumps(gw_message), headers=headers, ssl=False) as resp:
                if resp.status != 201:
                    mythic_sync_log.error(f"[!] Error posting to Ghostwriter in createEntry: {resp.status}\n")
                else:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.put(f"{GHOSTWRITER_URL}/oplog/api/entries/{entry_id}/?format=json", data=json.dumps(gw_message), headers=headers, ssl=False) as resp:
                if resp.status != 200:
                    mythic_sync_log.error(f"[!] Error posting to Ghostwriter in updateEntry: {resp.status}\n")
    except Exception as e:
...
async def handle_task(mythic_instance: mythic_classes.Mythic):
    ...
    async for data in mythic.subscribe_all_tasks_and_updates(mythic=mythic_instance, custom_return_attributes=custom_return_attributes):
        try:
            ...
            mythic_sync_log.error(f"[!] Failed to connect to Redis or process Mythic Task in handle_task: {str(e)}")
            continue
    ...
    while True:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(MYTHIC_URL, ssl=False) as resp:
                if resp.status != 200:
                    mythic_sync_log.info(f"[-] Got an HTTP {resp.status} response, trying again in {WAIT_TIMEOUT} seconds...")
                    await asyncio.sleep(WAIT_TIMEOUT)
                    continue
        return
...
async def wait_for_redis():
    ...
    while True:
        try:
            ...
            mythic_sync_log.info(f"[-] Failed to connect to redis at {REDIS_HOSTNAME}:{REDIS_PORT}, trying again in {WAIT_TIMEOUT} seconds...")
            await asyncio.sleep(WAIT_TIMEOUT)
            continue
    ...
    while True:
        if len(MYTHIC_API_KEY) == 0:
            ...
            try:
                ...
                mythic_sync_log.error(f"[-] Failed to Authenticate: {str(e)}, trying again in {WAIT_TIMEOUT} seconds...")
                await asyncio.sleep(WAIT_TIMEOUT)
                continue
            try:
            ...
            mythic_sync_log.error("[!] Must supply a MYTHIC_USERNAME and MYTHIC_PASSWORD\n")
            sys.exit(1)
        else:
            mythic_sync_log.info(f"[*] Authenticating to Mythic, https://{MYTHIC_IP}:{MYTHIC_PORT}, with a specified API Key")
            try:
                mythic_instance = await mythic.login(
                ...
                mythic_sync_log.error(f"[-] Failed to authenticate with API token: {e}, trying again in {WAIT_TIMEOUT} seconds...")
                await asyncio.sleep(WAIT_TIMEOUT)
                continue
            return mythic_instance
...
async def scripting():
    while True:
        ...
        try:
            ...
            mythic_sync_log.error(f"[-] Hit exception while subscribing to tasks and responses, {e}, restarting")
